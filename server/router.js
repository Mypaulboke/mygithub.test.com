...
exports.register = function(req, res, next) {
  ...
  form.parse(req, function(err, fields, files) {
    ...
    let member_id = fields.member_id;
    ...
    // 根据用户名查询数据库
    db.insertOne('users', newData, function(err, result) {
          if (err) {
            console.log(err)
            return res.json({
              "code": 404,
  ...
  form.parse(req, function(err, fields, files) {
    ...
    db.find('users', {query:{user:username}}, function(err, result) {
      ...
      if (dbPassword === password) {
        ...
        res.cookie('member_id', member_id, { maxAge: expires })
         res.cookie('nickname', member_id, { maxAge: expires })
        return res.json({
          "code": 200,
          "message": "登录成功",
          "result":result
...
//   let path = req.protocol + '://' + req.headers.host + '/public/avatars/';
//   let username = req.cookies.username;
//   let member_id =req.cookies.member_id;
//   let id = req.cookies.id
//   console.log(id);
//   db.find('users', {query:{"_id":id}}, function(err, result) {
//     if (err) {
//       console.log(err)
//       return res.json({
//         "code": 404,
...
exports.user = function(req, res, next) {
  ...
  let id = req.cookies.id;
  console.log(id);
  db.findById('users',{query:{_id:id}}, function(err, result) {
    if (err) {
      console.log(err)
      return res.json({
        "code": 404,
...
exports.avatar = function(req, res, next) {
  ...
  form.parse(req, function(err, fields, files) {
    ...
    // let extname = files.avatar.name
    let oldUrl = files.avatar.path;
    let newUrl = './public/avatars/'+username + extname;
    let avatarName = username + extname;
...
exports.deleteAccount = function(req, res, next) {
  ...
  db.deleteMany('users', { user: name }, function(err, result) {
    if (err) {
      console.log(err)
      return res.json({
        "code": 401,
